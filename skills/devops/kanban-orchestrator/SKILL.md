...
**Live UI triage → Kanban after verification:** first verify the route with terminal/browser. If the page loads but a section is missing or degraded, create a Kanban task for the integration/data bug rather than treating it as a total outage. See `references/mission-control-triage.md` for a Mission Control example.
...
**Don't infer a full outage from one browser complaint.** If terminal/browser checks show the route is alive, classify the problem as partial rendering, missing data, or client-side degradation and route the Kanban task accordingly.
